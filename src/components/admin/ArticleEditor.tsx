      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 space-y-5">
            {/* ─── Cover / Thumbnail ─── */}
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2 uppercase tracking-wide">Cover Thumbnail</label>
              {form.cover_image ? (
                <div className="relative rounded-xl overflow-hidden border border-gray-700 group">
                  <img src={form.cover_image} alt="Cover" className="w-full h-52 object-cover" />
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                    <label className="cursor-pointer px-3 py-1.5 bg-gray-900/80 hover:bg-gray-800 rounded-lg text-xs text-white font-medium flex items-center gap-1.5 border border-gray-600">
                      <input type="file" accept="image/*" className="hidden" disabled={uploadingCover}
                        onChange={async (e) => {
                          const file = e.target.files?.[0]; if (!file) return;
                          if (file.size > 5 * 1024 * 1024) { alert("Image must be under 5MB"); return; }
                          setUploadingCover(true);
                          const fd = new FormData(); fd.append("file", file); fd.append("bucket", "article-images"); fd.append("folder", "covers");
                    ...
                    <button type="button" onClick={() => setForm(f => ({ ...f, cover_image: "" }))}
                      className="px-3 py-1.5 bg-red-900/80 hover:bg-red-800 rounded-lg text-xs text-red-300 font-medium border border-red-700">Remove</button>
                  </div>
                </div>
              ) : (
                <label className={`flex flex-col items-center justify-center gap-2 w-full h-44 rounded-xl border-2 border-dashed cursor-pointer transition-colors ${uploadingCover ? "border-amber-400 bg-amber-900/10 pointer-events-none" : "border-gray-700 hover:border-gray-500 hover:bg-gray-800/50"}`}>
                  <input type="file" accept="image/*" className="hidden" disabled={uploadingCover}
                    onChange={async (e) => {
                      const file = e.target.files?.[0]; if (!file) return;
                      if (file.size > 5 * 1024 * 1024) { alert("Image must be under 5MB"); return; }
                  ...
                  {uploadingCover ? (
                    <><div className="w-6 h-6 border-2 border-amber-600 border-t-transparent rounded-full animate-spin" /><span className="text-sm text-amber-500 font-medium">Uploading…</span></>
                  ) : (
                    <>
                      ...
                      <span className="text-sm font-semibold text-gray-400">Click to upload cover image</span>
                      <span className="text-xs text-gray-600">PNG, JPG up to 5MB — shown as article hero</span>
                    </>
                  )}
                </label>
              )}
