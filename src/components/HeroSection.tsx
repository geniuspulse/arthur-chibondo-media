  ...
  return (
    <section className="max-w-6xl mx-auto px-6 py-20 sm:py-28 grid lg:grid-cols-2 gap-12 items-center">
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
        <span className="inline-block bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-sm font-semibold px-3 py-1 rounded-full mb-6">
        ...
        <div className="flex flex-wrap gap-3 mb-8">
          ...
          <Link
            href="/projects"
            className="inline-flex items-center gap-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:border-green-600 hover:text-green-600 font-semibold px-6 py-3 rounded-xl transition-colors"
          >
            Explore Projects
          </Link>
        </div>
        <div className="flex gap-3">
          {socials.map((s) => (
            <a
              ...
              href={s.href}
              aria-label={s.label}
              className="w-10 h-10 rounded-xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-green-100 dark:hover:bg-green-900/30 hover:text-green-600 transition-colors"
            >
              {s.icon}
            </a>
          ))}
      ...
      >
        <div className="relative">
          <div className="absolute inset-0 bg-green-600 rounded-2xl translate-x-3 translate-y-3 opacity-20" />
          <img
