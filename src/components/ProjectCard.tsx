  ...
  return (
    <div className="group rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-all bg-white dark:bg-gray-900 flex flex-col">
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <p className="text-sm text-amber-600 font-medium mb-3">{project.tagline}</p>
        <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-3 flex-1">{project.description}</p>
        <div className="mt-4 flex gap-3">
          <Link href={`/projects/${project.slug}`} className="text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-amber-600 transition-colors">
            Learn more →
          </Link>
          {project.website_url && (
            <a href={project.website_url} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-amber-600 flex items-center gap-1 hover:text-amber-500 transition-colors">
