      ...
      <div className="relative w-full h-72 sm:h-96 overflow-hidden bg-gray-100 dark:bg-gray-900">
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="max-w-3xl mx-auto px-6 pb-10 w-full">
            <span className="inline-block bg-green-600 text-white text-xs font-semibold px-3 py-1 rounded-full mb-3">{article.category}</span>
            <h1 className="text-2xl sm:text-4xl font-bold text-white font-serif leading-tight">{article.title}</h1>
          </div>
        </div>
      </div>
      <div className="max-w-3xl mx-auto px-6 py-12">
        <Link href="/articles" className="inline-flex items-center gap-2 text-sm text-green-600 hover:text-green-500 mb-8"><ArrowLeft size={16} /> Back to Blog</Link>
        ...
        {related && related.length > 0 && (
          <div className="mt-16">
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              {related.map((r: any) => (
                <Link key={r.id} href={`/articles/${r.slug}`} className="group p-4 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-green-500 transition-colors">
                  <span className="text-xs font-semibold text-green-600 block mb-1">{r.category}</span>
                  <p className="font-semibold text-gray-900 dark:text-white group-hover:text-green-600 transition-colors">{r.title}</p>
                </Link>
              ))}
            </div>
          </div>
