  ...
  return (
    <main className="min-h-screen">
      <section className="py-20 px-6 text-center bg-gray-50 dark:bg-gray-950 border-b border-gray-200 dark:border-gray-800">
        <span className="text-amber-600 text-sm font-semibold uppercase tracking-widest mb-4 block">Press & Media</span>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {items.map((item: any) => (
              <a key={item.id} href={item.url} target="_blank" rel="noopener noreferrer" className="group block rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-all">
                ...
                <div className="p-5">
                  <span className="text-xs font-semibold text-amber-600 block mb-2">{item.type}</span>
                  <h3 className="font-bold text-gray-900 dark:text-white mb-1 group-hover:text-amber-600 transition-colors">{item.title}</h3>
