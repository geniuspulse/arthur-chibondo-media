  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <span className="text-xs font-semibold text-amber-600 uppercase tracking-wider">Insights</span>
        ...
        <div className="flex gap-1 bg-gray-100 dark:bg-slate-800 rounded-lg p-1">
          {['7d', '30d', 'all'].map(r => (
            <button key={r} onClick={() => setRange(r)}
              className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all ${range === r ? 'bg-white dark:bg-slate-900 text-amber-600 shadow-sm' : 'text-gray-500'}`}>
      ...
      {data?.dailyTrend && data.dailyTrend.length > 0 && (
        <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
            <BarChart3 size={18} className="text-amber-600" /> Daily Views Trend
          </h2>
          <div className="flex items-end gap-1 h-48 overflow-x-auto">
            {data.dailyTrend.map(d => (
              <div key={d.date} className="flex flex-col items-center gap-1 flex-1 min-w-[40px]">
                ...
                <div className="w-full bg-amber-500 rounded-t-sm transition-all hover:bg-amber-600"
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <div>
          <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
            <FileText size={18} className="text-amber-600" /> Top Stories
          </h2>
        ...
        <div>
          <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
            <Globe size={18} className="text-amber-600" /> Traffic Sources
          </h2>
          <div className="space-y-2">
            {data?.referrers && data.referrers.length > 0 ? data.referrers.map((r, i) => (
              <div key={r.source} className="flex items-center justify-between p-3 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg">
                ...
                <span className="text-xs font-bold text-amber-600">{r.count}</span>
              </div>
            )) : (
              <p className="text-sm text-gray-400 p-3">No referrer data yet.</p>
            )}
          </div>
        </div>
      </div>
      ...
      {data?.uniqueVisitors === 0 && (
        <div className="p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/30 rounded-lg">
          ...
          <p className="text-xs text-amber-600 dark:text-amber-500 mt-1">
            Run the SQL shown in the error state in your Supabase SQL Editor to enable full analytics.
          </p>
        </div>
      )}
    </div>
...
function StatCard({ label, value, icon }: { label: string; value: any; icon: React.ReactNode }) {
  return (
    <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6 shadow-sm flex items-center gap-5">
      <div className="p-3 rounded-lg bg-amber-50 dark:bg-amber-900/10 text-amber-600 dark:text-amber-400">{icon}</div>
      <div>
