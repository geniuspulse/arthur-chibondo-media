...
export default function AdminDashboard() {
  ...
  const statCards = [
    ...
    { label: "Subscribers", value: stats.subscribers, sub: "Active newsletter subs", icon: <Mail size={20} />, href: "/admin/subscribers", color: "text-blue-600 bg-blue-50 dark:bg-blue-900/20" },
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {statCards.map((card) => (
          <Link key={card.label} href={card.href} className="group bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 hover:shadow-md transition-all hover:border-blue-300 dark:hover:border-blue-700">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            ...
            <Link href="/admin/articles" className="text-xs text-blue-600 hover:text-blue-500 font-medium">View all →</Link>
          </div>
            ...
            {recentArticles.map((a) => (
              <Link key={a.id} href={`/admin/articles/${a.id}`} className="flex items-center gap-3 px-6 py-3 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${a.status === "published" ? "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400" : a.status === "draft" ? "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400" : "bg-gray-100 text-gray-500 dark:bg-gray-800"}`}>
                    {a.status}
                  </span>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            ...
            <Link href="/admin/messages" className="text-xs text-blue-600 hover:text-blue-500 font-medium">View all →</Link>
          </div>
          <div className="divide-y divide-gray-100 dark:divide-gray-800">
              ...
              <Link key={m.id} href={`/admin/messages`} className="flex items-start gap-3 px-6 py-3 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    ...
                    {m.status === "new" && <span className="w-2 h-2 rounded-full bg-blue-500 flex-shrink-0 ml-2" />}
                  </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { href: "/", label: "View Site", icon: <Eye size={18} />, color: "bg-blue-50 dark:bg-blue-900/20 text-blue-600", external: true },
          ].map((action) => (
            <Link
              key={action.label}
              href={action.href}
