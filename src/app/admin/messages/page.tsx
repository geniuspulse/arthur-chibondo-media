    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search messages..." className="w-full pl-9 pr-4 py-2.5 text-sm rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-600" />
        </div>
        {["all", "new", "read", "replied"].map(s => (
          <button key={s} onClick={() => setStatusFilter(s)}
            className={`px-4 py-2.5 rounded-xl text-sm font-medium capitalize transition-all ${statusFilter === s ? "bg-green-600 text-white" : "bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:border-green-500"}`}>
            {s}
          </button>
        ))}
      </div>
            ...
            <div className="divide-y divide-gray-100 dark:divide-gray-800 max-h-[600px] overflow-y-auto">
              {filtered.map(m => (
                <button key={m.id} onClick={() => openMessage(m)}
                  className={`w-full text-left px-5 py-4 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors ${selected?.id === m.id ? "bg-green-50 dark:bg-green-900/10 border-l-2 border-l-green-500" : ""}`}>
                  <div className="flex items-center gap-2 mb-1">
                    {m.status === "new" ? <Mail size={13} className="text-green-600 flex-shrink-0" /> : <MailOpen size={13} className="text-gray-400 flex-shrink-0" />}
            ...
            <div className="p-6 space-y-5 h-full flex flex-col">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">{selected.name} · <a href={`mailto:${selected.email}`} className="text-green-600 hover:underline">{selected.email}</a></p>
              ...
              <a
                ...
                className="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors w-fit"
              >
