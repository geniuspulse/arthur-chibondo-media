...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const { data: project } = await supabase.from("projects").select("name, tagline, description").eq("slug", params.slug).single();
  if (!project) return { title: "Project Not Found" };

  return {
    title: project.name,
    description: project.tagline || project.description || "APM Chibondo project",
    openGraph: {
      title: project.name,
      description: project.tagline || project.description || "APM Chibondo project",
...
export default async function ProjectPage({ params }: { params: { slug: string } }) {
  ...
  const statusStyles: Record<string, string> = {
    active: "bg-gray-800 text-gray-300 border border-gray-700",
    in_development: "bg-gray-800 text-gray-300 border border-gray-700",
    completed: "bg-gray-800 text-gray-300 border border-gray-700",
    coming_soon: "bg-gray-800 text-gray-300 border border-gray-700",
  };
  ...
  return (
    <main>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        <Link href="/projects" className="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-white transition-colors mb-10"><ArrowLeft size={16} /> Back to Projects</Link>
        ...
        {project.website_url && (
          <a href={project.website_url} target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-6 py-3 bg-white hover:bg-gray-100 text-gray-900 font-semibold rounded-xl transition-colors">
